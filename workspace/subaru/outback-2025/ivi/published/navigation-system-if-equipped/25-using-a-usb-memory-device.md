...
<div class="fn-meta"><b>Function path:</b> Navigation System (If equipped) / Using A USB Memory Device<br><b>Source:</b> printed page 224, 225<br><b>Test-ready:</b> <span class="test-ready-yes">yes — no unfilled thresholds and a procedure is present</span></div>
...
```mermaid
flowchart TD
    S1["1. Connect to a USB memory device. (→P.70)"]
    ...
    S7["1. Connect to a Wi-Fi® network. (→P.96)"]
...
| 2 | 2 | Select (Map Update) on the navigation settings screen. (→P.219) | p.224 / step |
| 3 | 3 | → (USB) USB | p.224 / step |
| 4 | 4 | After the updates have been completed, turn the ignition switch to the “LOCK”/“OFF” position and exit the vehicle. | p.224 / step |
| 5 | 5 | Close the driver’s door and lock the doors, then move at least 10 feet (3 m) away from the vehicle to prevent the key from interfering. | p.224 / step |
| 6 | 6 | After at least 5 minutes have elapsed, start the engine again. | p.224 / step |
| 7 | 1 | Connect to a Wi-Fi® network. (→P.96) | p.225 / step |
...
|---|---|---|---|
| 1 | <span class="req-label">Using A USB Memory Device</span>Maps can be updated using a USB memory device. | capability | p.224 / text |
| 2 | <span class="req-label">Using A USB Memory Device</span>For details and to download update data, refer to the following URL: https://subaru-maps.com/. | capability | p.224 / text |

## 25-2-2. Service requirements

| # | Presumed requirement | Strength | Source |
|---|---|---|---|
| 1 | <span class="req-label">Step -</span>The new map data will be applied. | capability | p.224 / bullet |
| 2 | <span class="req-label">Using A USB Memory Device</span>UPDATING THE MAP DATA MANUALLY USING Wi-Fi®. | capability | p.225 / text |
| 3 | <span class="req-label">Using A USB Memory Device</span>The map data may also be updated via Wi-Fi® network. | capability | p.225 / text |
