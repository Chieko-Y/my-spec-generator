...
|---|---|---|---|
| 1 | <span class="req-label">Bluetooth phone message function</span>Received messages can be forwarded from the connected Bluetooth phone, enabling checking and replying using the system. | capability | p.83 / text |
| 2 | <span class="req-label">Bluetooth phone message function</span>Depending on the type of Bluetooth phone connected, received messages may not be transferred to the message inbox. | capability | p.83 / text |
| 3 | <span class="req-label">Bluetooth phone message function</span>If the phone does not support the message function, this function cannot be used. | constraint | p.83 / text |
| 4 | <span class="req-label">Bluetooth phone message function</span>Depending on the Bluetooth phone type, the screen display may differ, and it may not be possible to use certain functions. | capability | p.83 / text |
| 5 | <span class="req-label">Bluetooth phone message function</span>To use this function, it is necessary to enable “Sync” for messages on the phone settings screen. (→P.45). | capability | p.83 / text |
